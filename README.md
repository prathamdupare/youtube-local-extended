# Youtube Local - Extended

![screenshot](images/screenshot.png)
**youtube-local-extended** is a browser-based client written in Python, and it's a fork of the original youtube-local project. It allows you to watch YouTube anonymously and without the lag of the slow official YouTube web page. One of its primary features is that it routes all requests through Tor (although you have the option to disable Tor routing), except for the video files hosted on googlevideo.com. This approach is similar to what HookTube (now defunct) and Invidious do, but with the advantage that you don't have to trust a third-party service to respect your privacy. The underlying assumption is that Google is unlikely to put in the effort to incorporate video file requests into their tracking, given the relatively small number of users who prioritize privacy. Please note that while Tor routing provides anonymity, it may result in higher latency compared to regular YouTube. However, when not routing through Tor, video pages may load faster than they do on the official YouTube website, depending on your browser.
